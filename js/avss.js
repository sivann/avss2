...
function playFiles(files,method,order) {
	...
	if (method == 'm3u') {
		...
		//jplaylist_play(files[0]);
		jplaylist_play(files);
	}
}
	...
	jplayer1.jPlayer({
		ready: function () {
			$(this).jPlayer("setMedia", {
				title: fstr,
				//m4a: url
				m4a: "http://mute.netmode.ece.ntua.gr/avss2/index.php?action=getfile&path=%2Fmusic%2Fartists%2FT%2Fthe+doors%2Fan+american+prayer&file=the+doors+-+angels+and+sailors+-+08.mp3"
...
function jplaylist_play(files) {
	...
	for (i in files) {
		...
		url= "http://mute.netmode.ece.ntua.gr/avss2/index.php?action=getfile&file="+f.params.file+"&path="+f.params.path
